...
def load_and_cache_examples(config, task, tokenizer, evaluate=False, test=False):
    ...
    if os.path.exists(cached_features_file):
        ...
        examples = (
            processor.get_dev_examples(config.data_dir)
            if evaluate
            else processor.get_train_examples(config.data_dir)
        )
...
def main():
    ...
    ## resize the token embeddings in accordance with the new masks.
